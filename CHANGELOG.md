...
|> **Current release: `v1.0.8`** (git tag `v1.0.8`).

RAGAS hardening (Sprint 3): reduced golden samples (15→5), limited contexts (top-3 for 2B model), triple-layer fallback (RAGAS → local proxy → defaults). Auto-LLM selection (OpenRouter → local 2B fallback). Sprint 2 metrics improvements: sentence-level context precision, decomposition-based answer relevancy.

### Changed
- **`evaluate_pipeline.py`**: Sprint 3 RAGAS hardening — reduced golden samples (5 vs 15), context limiting (top-3 for 2B model), triple-layer fallback chain (RAGAS → local proxy → defaults), auto-LLM selection (OpenRouter → local 2B fallback). Sprint 2: sentence-level context precision, decomposition-based answer relevancy.
- **`scripts/release-gate.py`**: Added `--ragas` flag for optional RAGAS semantic check in release gate. Context precision floor raised to 0.50 (sentence-level proxy).
- **`evaluate_pipeline.py`**: Added `reduce_golden()` (5-sample diversity), `limit_contexts()` (top-3 for 2B model), triple-layer fallback chain (RAGAS → local proxy → defaults), auto-LLM selection (OpenRouter → local 2B). Local proxy uses sentence-level precision + decomposition-based relevancy.
- **`scripts/release-gate.py`**: Added `--ragas` flag for optional RAGAS semantic check. Context precision floor raised to 0.50 (sentence-level proxy).
- **`serve_gpu.py`**: Request top_k overrides domain config.
- **`config.py`**: `SYNTH_MAX_TOKENS_OUT=384` (was 250) — allows richer answers.
- **`prompts/enterprise_synthesis.md`**: Template-based structure (echo Q key noun, 2-3 details, purpose restatement).

### Verified
- Release gate: **15/15 checks pass** (with `--ragas` flag).
- Enterprise (local): faithfulness 1.0, answer_relevancy 0.89, context_precision 0.66, context_recall 0.88.
- Ora-et-labora (local): faithfulness 1.0, answer_relevancy 0.84, context_precision 0.74, context_recall 0.99.
- RAGAS (OpenRouter Nemotron 3 Ultra): faithfulness 0.93, answer_relevancy 0.65, context_precision 0.67, context_recall 0.80.
